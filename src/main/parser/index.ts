...
export type ParserReader<T> = Reader<
  { prompt: VorpalCommand; config: ParserConfig },
  T
>;
...
export default function(vorpal: any) {
  ...
  vorpal
    ...
    .action(async function(this: VorpalCommand, args: any) {
      configGet(args.key).run({
        prompt: this,
        config: ParserConfig.getOrCreate()
      });
    });

  vorpal
    .command(
      "parser config set <key> <value>",
      "Set a config value for the parser"
    )
    .alias("parser config-set", "parser set-config")
    .action(async function(this: VorpalCommand, args: any) {
      configSet(args.key, args.value).run({
        prompt: this,
        config: ParserConfig.getOrCreate()
      });
    });
  ...
  vorpal
    .command(
      "parser deps list",
      "List the install dependencies in your project"
    )
